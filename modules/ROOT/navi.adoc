:includedir: ../../
include::{includedir}modules/ROOT/partials/defines/define-all-include.adoc[]
ifdef::env-vscode[]
:imagesdir: ../../
endif::env-vscode[]
// ここまで空行禁止(無駄な改行があると設定が有効になりません）

// 1on1ミーティングガイドの目的と背景
* xref:1on1ガイドの目的と背景.adoc[{name-1on1guide-raw}の目的と背景]
// 1on1の目的
* xref:1on1の目的.adoc[{name-1on1-raw}の目的]
// 1on1の前提条件
* xref:1on1の前提条件.adoc[{name-1on1-raw}の前提条件]
// 1on1の価値基準
* xref:1on1の価値基準.adoc[{name-1on1-raw}の価値基準]
// 価値基準を満たすための行動
* xref:価値基準を満たすための行動.adoc[価値基準を満たすための行動]
// 本書の構成
* xref:本書の構成.adoc[本書の構成]
// 記載内容について
** xref:記載内容について.adoc[記載内容について]
// カテゴリー
** xref:カテゴリー.adoc[カテゴリー]
// パターン
//* xref:1on1ミーティングパターン.adoc[{name-1on1meeting-raw}パターン]
* 1on1ミーティングパターン
** xref:patterns/1.adoc[]
** xref:patterns/2.adoc[]
** xref:patterns/3.adoc[]
** xref:patterns/4.adoc[]
** xref:patterns/5.adoc[]
** xref:patterns/6.adoc[]
** xref:patterns/7.adoc[]
** xref:patterns/8.adoc[]
** xref:patterns/9.adoc[]
** xref:patterns/10.adoc[]
** xref:patterns/11.adoc[]
** xref:patterns/12.adoc[]
** xref:patterns/13.adoc[]
** xref:patterns/14.adoc[]
** xref:patterns/15.adoc[]
** xref:patterns/16.adoc[]
** xref:patterns/17.adoc[]
** xref:patterns/18.adoc[]
** xref:patterns/19.adoc[]
** xref:patterns/20.adoc[]
** xref:patterns/21.adoc[]
** xref:patterns/22.adoc[]
** xref:patterns/23.adoc[]
** xref:patterns/24.adoc[]
// 用語集
* xref:用語集.adoc[用語集]
//ライセンス
* xref:LICENSE.adoc[ライセンス]