...
define-literal.adocに定義された情報は「\{name-this-guide}」のように記載することで「{name-this-guide}」と出力したり、
...
自動化した出力では利用しない内容
(「convert-for-readers」は本サンプルで定義している項目)
\endif::[]
...
.ブロック画像
image::contents/images/1on1_Meeting_Patterns.jpg[pageTop,{full-page}]
...
インライン {icon-sample-check} で表示

image:contents/images/SvgBadges/badges/badge-user-sample-check.svg[]
...
本ドキュメントは{zwsp}{link-asciidoc-syntax-reference-html}で閲覧できます。 +
\{zwsp}は日本語との結合時に必要です。
